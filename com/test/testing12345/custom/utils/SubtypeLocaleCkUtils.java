...
public final class SubtypeLocaleCkUtils {
    ...
    public static final String LAYOUT_UZBEK = "uzbek";
     ...
     public static List<SubtypeCk> getSubtypes(final String locale, final Resources resources) {
        return new SubtypeBuilder(locale, true, resources).getSubtypes();
    }

     public static SubtypeCk getDefaultSubtype(final String locale, final Resources resources) {
        final List<SubtypeCk> subtypeCks = new SubtypeBuilder(locale, true, resources).getSubtypes();
        return subtypeCks.size() == 0 ? null : subtypeCks.get(0);
    }

     public static SubtypeCk getSubtype(final String locale, final String layoutSet,
                                        final Resources resources) {
        final List<SubtypeCk> subtypeCks =
                new SubtypeBuilder(locale, layoutSet, resources).getSubtypes();
        return subtypeCks.size() == 0 ? null : subtypeCks.get(0);
    }


    public static List<SubtypeCk> getDefaultSubtypes(final Resources resources) {
        ...
        for (final String localeString : sSupportedLocales) {
            supportedLocales.add(LocaleCkUtils.constructLocaleFromString(localeString));
        }

        final List<Locale> systemLocales = LocaleCkUtils.getSystemLocales();

        final ArrayList<SubtypeCk> subtypeCks = new ArrayList<>();
        final HashSet<Locale> addedLocales = new HashSet<>();
        for (final Locale systemLocale : systemLocales) {
            final Locale bestLocale = LocaleCkUtils.findBestLocale(systemLocale, supportedLocales);
            if (bestLocale != null && !addedLocales.contains(bestLocale)) {
                addedLocales.add(bestLocale);
                final String bestLocaleString = LocaleCkUtils.getLocaleString(bestLocale);
                subtypeCks.add(getDefaultSubtype(bestLocaleString, resources));
            }
        }
        if (subtypeCks.size() == 0) {
            // there needs to be at least one default subtype
            subtypeCks.add(getSubtypes(LOCALE_ENGLISH_UNITED_STATES, resources).get(0));
        }
        return subtypeCks;
    }
    ...
    private static class SubtypeBuilder {
        ...
        private List<SubtypeCk> mSubtypeCks;
        ...
        public List<SubtypeCk> getSubtypes() {
            if (mSubtypeCks != null) {
                return mSubtypeCks;
            }
            mSubtypeCks = new ArrayList<>();
         ...
         private void addLayout(final String keyboardLayoutSet) {
            ...
            mSubtypeCks.add(
                    new SubtypeCk(mLocale, keyboardLayoutSet, layoutNameStr, false, mResources));
        }
         ...
         private void addLayout(final String keyboardLayoutSet, final int layoutRes) {
            ...
            mSubtypeCks.add(
                    new SubtypeCk(mLocale, keyboardLayoutSet, layoutRes, true, mResources));
        }

         private void addGenericLayouts() {
            if (mSubtypeCks.size() > 0 && !mAllowMultiple) {
                return;
            }
            final int initialSize = mSubtypeCks.size();
            ...
            for (int i = 0; i < predefinedKeyboardLayoutSets.length; i++) {
                ...
                for (int subtypeIndex = 0; subtypeIndex < initialSize; subtypeIndex++) {
                    final String layoutSet = mSubtypeCks.get(subtypeIndex).getKeyboardLayoutSet();
                ...
                mSubtypeCks.add(new SubtypeCk(mLocale, predefinedLayout,
